...
def read_video(path_video):
    ...
    if not cap.isOpened():
        raise ValueError(f"Could not open video file: {path_video}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_input_video', type=str, help='path to input video', required=True)
    parser.add_argument('--path_output_video', type=str, help='path to output video', required=True)
    args = parser.parse_args()

    if not args.path_input_video:
        raise ValueError("The path to the input video must be provided.")
    if not os.path.exists(args.path_input_video):
        raise FileNotFoundError(f"Input video file not found: {args.path_input_video}")
